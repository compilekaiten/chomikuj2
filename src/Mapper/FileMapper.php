...
class FileMapper implements FileMapperInterface {
    ...
    private function extractFileFromSearchRow(\DOMXpath $xpath, \DOMNode $contextNode): File {
        ...
        return new File($name, $path, $this->convertTextualSizeToKilobytes($size), $this->convertTextualTimeUploadedToDateTime($timeUploaded));
    }
    ...
    private function extractFileSizeFromSearchRow(\DOMXpath $xpath, \DOMNode $contextNode): string {
        ...
        foreach ($fileInfoNodes as $node) {
            preg_match('/^([0-9]+\ [A-Z]{2}|[0-9]+,[0-9]+\ [A-Z]{2})$/', (string) $node->nodeValue, $matches);
    ...
    private function convertTextualSizeToKilobytes(string $textualSize) {
        $textualSize = explode(' ', $textualSize);

        $value = floatval(str_replace(',', '.', $textualSize[0]));
    ...
    private function convertTextualTimeUploadedToDateTime(string $texttualTimeUploaded): \DateTime {
        ...
        $date = \DateTime::createFromFormat('j M y G:i', $texttualTimeUploaded, new \DateTimeZone('Europe/Warsaw') // important, because Chomikuj always displays time using Europe/Warsaw timezone
        );
